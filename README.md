...
GPT and other tool-capable routes receive native MCP function schemas when MCP is requested. OpenAI-compatible endpoint bases such as `/v1` are normalized to `/v1/chat/completions` at dispatch, so old or direct-created sessions do not POST to the proxy root. For chat-style fallback models that do not receive native MCP schemas, Odysseus waits briefly for requested MCP servers to finish connecting before it builds the agent prompt. Multiagent mode keeps MCP schemas available for real sequential handoffs, but concrete follow-up turns are scoped to the latest user instruction so a stale MCP request does not leak into a Bash-only or tool-free turn. Text-tool models are prompted to emit MCP calls as raw lines like `mcp__server_id__tool_name{"required_arg":"real value"}`; Odysseus also accepts the common fallback form `mcp__server_id__tool_name({"required_arg":"real value"})`. Empty `{}` calls and blank required arguments are rejected instead of being forwarded. If an MCP search call arrives with a missing query, or with extra neighboring text despite an explicit "exact query", Odysseus repairs the query from the current real user/workflow instruction before dispatch. If a native GPT Bash call arrives with empty arguments but the user explicitly supplied a Bash command, Odysseus repairs the command before execution. Exact duplicate tool calls in the same model round are dropped, exact Bash commands must actually execute before the turn is accepted, non-matching Bash calls are blocked when an exact command is in scope, and turns that explicitly say "do not call tools" run with no native schemas and discard any attempted tool block before execution. API-token chat/session requests are attributed to the token owner, not the synthetic `api` user, so owner-scoped endpoints and admin-visible MCP tools stay available to paired clients. See [Agent MCP smoke testing](docs/agent-mcp-smoke.md) for the live GPT + MCP + Bash regression checks, including sequential group handoff.
...
Sequential mode is strict order: the first participant receives the user prompt, and each following participant receives a structured handoff whose primary input is the previous participant's cleaned artifact. The handoff also includes bounded original-task context and a bounded tool trace, so a tool-heavy agent does not leave the next checker with an empty prompt. The injected group prompt tells agents to call tools with explicit arguments, redact sensitive internal details, and produce role-specific artifacts that checker/reviewer agents can validate into an execution plan.
