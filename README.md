...
GPT and other tool-capable routes receive native MCP function schemas when MCP is requested. OpenAI-compatible endpoint bases such as `/v1` are normalized to `/v1/chat/completions` at dispatch, so old or direct-created sessions do not POST to the proxy root. For chat-style fallback models that do not receive native MCP schemas, Odysseus waits briefly for requested MCP servers to finish connecting before it builds the agent prompt. Multiagent mode keeps MCP schemas available even when a later sequential handoff no longer repeats the word "MCP". Text-tool models are prompted to emit MCP calls as raw lines like `mcp__server_id__tool_name{"required_arg":"real value"}`; Odysseus also accepts the common fallback form `mcp__server_id__tool_name({"required_arg":"real value"})`. Empty `{}` calls and blank required arguments are rejected instead of being forwarded. If an MCP search call still arrives with a missing query argument, either as native `tool_calls` or raw text, Odysseus repairs that search query from recent real user/workflow instructions before dispatching it. If a native GPT Bash call arrives with empty arguments but the user explicitly supplied a Bash command, Odysseus repairs the command before execution. API-token chat/session requests are attributed to the token owner, not the synthetic `api` user, so owner-scoped endpoints and admin-visible MCP tools stay available to paired clients. See [Agent MCP smoke testing](docs/agent-mcp-smoke.md) for the live GPT + MCP + Bash regression checks, including sequential group handoff.
